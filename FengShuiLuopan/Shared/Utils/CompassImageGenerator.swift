...
class CompassImageGenerator {
    ...
    static func generateCompassImage(size: CGFloat = 1000) -> UIImage {
        ...
        let image = renderer.image { context in
            drawCompass(context: context.cgContext, size: size)
        }
        return image
    }

    /// 绘制罗盘内容
    private static func drawCompass(context ctx: CGContext, size: CGFloat) {
        ...
        // 绘制24山刻度和文字
        draw24Mountains(context: ctx, center: center, radius: radius)
        ...
        // 绘制正北指示（红色三角形）
        drawNorthIndicator(context: ctx, center: center)
    }

    /// 绘制24山刻度和文字
    private static func draw24Mountains(context ctx: CGContext, center: CGPoint, radius: CGFloat) {
        for i in 0..<24 {
    ...
    /// 绘制正北指示
    private static func drawNorthIndicator(context ctx: CGContext, center: CGPoint) {
        ctx.setFillColor(UIColor.red.cgColor)
